    ...
    <View style={styles.filtroContainer}>
      {periodos.map((p) => (
        <TouchableOpacity
          ...
          style={[
            styles.filtroBotao,
            { backgroundColor: periodoAtivo === p.id ? theme.primary : theme.card },
          ]}
          onPress={() => onPeriodoChange(p.id)}
        >
          <Text
            style={[styles.filtroTexto, { color: periodoAtivo === p.id ? 'white' : theme.subtext }]}
          >
            {p.label}
          </Text>
        </TouchableOpacity>
        ...
        <>
          <View style={styles.statsGrid}>
            <StatCard
              icon="cash-outline"
              label="Faturamento"
              value={Number(stats.total_faturamento || 0).toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL',
              })}
              color="#10B981"
              tooltip="Soma total do valor de todos os serviços concluídos no período selecionado."
            />
            <StatCard
              ...
              color="#3B82F6"
              tooltip="Número total de agendamentos com status 'concluído' no período."
            />
            <StatCard
              icon="pricetag-outline"
              label="Ticket Médio"
              value={Number(stats.ticket_medio || 0).toLocaleString('pt-BR', {
                style: 'currency',
                currency: 'BRL',
              })}
              color="#F97316"
              tooltip="Valor médio que cada cliente gastou. Calculado dividindo o Faturamento pelo número de Agendamentos."
            />
            <StatCard
              icon="star-outline"
              label="Serviço Popular"
              value={stats.servico_mais_popular?.nome || '-'}
              color="#8B5CF6"
              tooltip="O serviço que foi agendado mais vezes no período selecionado."
            />
          </View>
            ...
            {servicoData.length > 0 ? (
              ...
              <View style={styles.placeholder}>
                ...
                <Text style={{ color: theme.subtext, marginTop: 10 }}>
                  Sem dados de serviços para exibir.
                </Text>
              </View>
            )}
          </View>
