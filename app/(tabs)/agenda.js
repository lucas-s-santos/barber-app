...
export default function AgendaScreen() {
  ...
  const gerarHorarios = useCallback(async (dataSelecionada) => {
    ...
    const { data, error } = await supabase.rpc('get_horarios_disponiveis', {
      p_barbearia_id: '9fa53483-b3ef-4460-b5cb-2593c439733d',
    ...
    if (error) {
      ...
      const horarios = data.map(item => item.horario_disponivel.substring(0, 5));
  ...
  const handleAgendar = async () => {
    ...
    const { data: barbeiroDisponivel, error: barbeiroError } = await supabase
      .from('barbeiros')
      .select('id')
      .eq('barbearia_id', '9fa53483-b3ef-4460-b5cb-2593c439733d')
      .limit(1)
      .single();

    if (barbeiroError || !barbeiroDisponivel) {
      Alert.alert("Erro", "Nenhum barbeiro encontrado para este agendamento.");
      return;
    }
    ...
    // Removida a linha 'barbearia_id', pois a coluna não existe mais na tabela 'agendamentos'.
    // =================================================================
    const { error } = await supabase.from('agendamentos').insert({
      cliente_id: user.id,
      barbeiro_id: barbeiroDisponivel.id,
    ...
    if (error) {
      ...
      Alert.alert("Agendamento Confirmado!", `Seu horário para ${servico.nome} foi marcado com sucesso.`);
      gerarHorarios(selectedDate);
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      ) : <Text style={styles.placeholderText}>Selecione uma data no calendário</Text>}
    </View>
  );
}
